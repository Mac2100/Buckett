#!/bin/bash
# Builds Buckett.app (universal binary) from the Swift package and wraps it in a DMG.
# Output: dist/Buckett.app and dist/Buckett-<version>.dmg
set -euo pipefail
cd "$(dirname "$0")/.."

APP_NAME="Buckett"
BUNDLE_ID="com.mac2100.Buckett"
MIN_MACOS="14.0"

VERSION=$(sed -n 's/.*marketing = "\([^"]*\)".*/\1/p' Sources/Buckett/Support/AppVersion.swift)
if [ -z "$VERSION" ]; then
